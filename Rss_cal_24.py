...
# =====================================
Pd_alpha = pd.read_csv("all_rss24.csv", header=None).iloc[:, 0].values
...
assert len(distances) == len(Pd_alpha), "distances"
...
# =====================================
Pd_sigmoid = pd.read_csv("all_rsss24.csv", header=None).iloc[:, 0].values
Sh_array   = pd.read_csv("all_thr24.csv",  header=None).iloc[:, 0].values
...
plt.grid(True)
plt.tight_layout()
plt.savefig("output1/alpha_fit.png")
plt.close()
...
plt.grid(True)
plt.tight_layout()
plt.savefig("output1/sigmoid_fit.png")
plt.close()

# 保存参数
with open("output1/fitted_parameters.txt", "w") as f:
